...
/**
 * from esbuild-register issue
 * @issue https://github.com/egoist/esbuild-register/issues/62
 * @refer https://github.com/stephenh/esbuild-register-circular
 */
test('circular deps', async () => {
  const { stdout } = await execa('node', [
    '-r',
    `${process.cwd()}/register.js`,
    `${process.cwd()}/tests/esbuild-register-circular/src/index.ts`,
  ])
  assert.equal(
    stdout.trim(),
    `today:${' '}
...
const ZX_PRO_V6_EXPECT_OUTPUT = `$ echo "123"
123
red []`
...
test('zx-pro v6', async () => {
  ...
  assert.equal(stdout.trim(), ZX_PRO_V6_EXPECT_OUTPUT)
})
